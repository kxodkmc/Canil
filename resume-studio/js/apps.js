...
RS.apps = (function () {
  ...
  let maskEl, panelEl, bodyEl, subEl, addBtn, stripEl;
  ...
  function renderCur() {
    ...
    if (!apps.length) {
      bodyEl.innerHTML = '<div class="ap-empty">'
        ...
        + '<div>还没有投递记录</div>'
        + '<div class="ap-empty-sub">点下方「＋ 添加一条投递」，记下岗位名称和招聘页网址，方便之后复盘。</div>'
        + '</div>';
      return;
    }
    bodyEl.innerHTML = apps.map(a =>
      ...
      +    '<input class="ap-job-input" data-field="job" value="' + esc(a.job) + '" placeholder="岗位名称，如：产品经理实习生（某公司）">'
      ...
      +    '<label class="ap-f"><span>招聘页网址</span><input class="ap-url" data-field="url" value="' + esc(a.url) + '" placeholder="可留空"></label>'
      +    '<label class="ap-f ap-f-full"><span>备注</span><input data-field="note" value="' + esc(a.note || "") + '" placeholder="进展 / 内推人 / 渠道等（可选）"></label>'
      +  '</div>'
      +  (a.url ? '<div class="ap-meta"><a href="' + esc(a.url) + '" target="_blank" rel="noopener">🔗 打开招聘页</a></div>' : "")
      + '</div>').join("");
  }
  ...
  function layoutMap(rows) {
    ...
    const jobs = rows.map(r => {
      const h = r.note ? MAP.jobHN : MAP.jobH;
  ...
  function renderAll() {
    ...
    const stat = '<div class="ap-stat">共 <b>' + rows.length + '</b> 条投递'
      + ' · 最近 7 天 <b>' + in7 + '</b> 条'
      + ' · 最近 30 天 <b>' + in30 + '</b> 条'
      + ' · 涉及 <b>' + new Set(rows.map(r => r.vid)).size + '</b> 个简历版本</div>';
  ...
  function renderStrip() {
    ...
    const chips = sorted.slice(0, MAX).map(a =>
      '<button class="as-chip" data-action="strip-open" title="' + esc((a.date || "") + " " + a.job) + '">'
      + esc(a.job) + '</button>').join("");
